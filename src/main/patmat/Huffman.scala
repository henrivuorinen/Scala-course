...
object Huffman {
   ...
   */
    def encode(tree: CodeTree)(text: List[Char]): List[Bit] = {
      def determinateDirection(fork: Fork, character: Char) : Direction = {
        fork.left match {
      ...
      def endcodeRec(currentTree: CodeTree, text: List[Char]) : List[Bit] = {
        if (text.isEmpty) List()
        else currentTree match {
          case Leaf(_, _) => endcodeRec(tree, text.tail)
          case f @ Fork(l, r, c, _) => determinateDirection(f, text.head) match {
            case Left => 0 :: endcodeRec(l, text)
            case Right => 1 :: endcodeRec(r, text)
          }

        }
      }
      endcodeRec(tree, text)
    }
